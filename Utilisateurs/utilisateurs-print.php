...
$r = "SELECT idutilisateur, nom, prenom, email, telephone, dateajout 
FROM utilisateurs";
...
$pdf->Cell(25, 10, 'Date Ajout', 1, 0, 'C', true);
$pdf->Ln();

// Afficher les données de la table
$pdf->SetFont('Arial', '', 9);
while ($data = mysqli_fetch_assoc($res)) {
    $pdf->Cell(15, 10, $data['idutilisateur'], 1);
    $pdf->Cell(35, 10, substr($data['nom'], 0, 16), 1);
    $pdf->Cell(35, 10, substr($data['prenom'], 0, 16), 1);
    $pdf->Cell(50, 10, substr($data['email'], 0, 22), 1);
    $pdf->Cell(30, 10, substr($data['telephone'], 0, 12), 1);
    $pdf->Cell(25, 10, $data['dateajout'], 1);
    $pdf->Ln();
}
